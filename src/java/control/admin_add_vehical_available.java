 ...
 * @author taola
 */
@WebServlet(name = "admin_add_vehical_available", urlPatterns = {"/admin_add_vehical_available"})
public class admin_add_vehical_available extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        request.getRequestDispatcher("admin_add_vehical_available.jsp").forward(request, response);
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        log(pid + "\n" + vid + "\n" + bid + "\n" + vquantity + "\n"
                + vprice + "\n" + vstatus + "\n"
                + vimg);
        if (dao.getVehicalInforById(pid, vid, bid) == null) {
            dao.insertVehicalAvailable(pid, vid, bid, vstatus, vquantity, vprice, vimg);
            request.getRequestDispatcher("admin_vehical_available").forward(request, response);
        } else {
            request.setAttribute("mess", "This one Existed");
            Vector<Tours> tours = dao.getAllTours();
            Vector<Vehical> vehicalT = dao.getAllVehicalType();
            Vector<Vehical_Brand> brands = dao.getAllVehicalsBrand();
            request.setAttribute("tours", tours);
